    ...
    def __call__(self, target, width, height):
        ...
        for obj in target:
            if 'bbox' in obj:
                ...
                final_box = list(np.array([bbox[0], bbox[1], bbox[0]+bbox[2], bbox[1]+bbox[3]])/scale) #將坐標值範圍縮到[0,1]
...
class COCODetection(data.Dataset):
    ...
    def pull_item(self, index):
        ...
        # 獲得指定圖像索引的標注信息，返回一個列表，列表的每一個值表示一個目標的標注信息
        if self.has_gt:
        ...
        img = cv2.imread(path)  # rgb information
